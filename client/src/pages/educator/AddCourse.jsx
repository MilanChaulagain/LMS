...
const AddCourse = () => {
  ...
  const handleLecture = (action, chapterId, lectureIndex) => {
    console.log("Handle Lecture is called...")
    if(action === 'add') {
      ...
      setChapters(
        chapters.map((chapter)=> {
          if(chapter.chapterId === chapterId) {
            chapter.chapterContent.filter(lectureIndex, 1);
          }
          return chapter;
        })
      )
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      ...
      className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input type="text" onChange={e => setCourseTitle(e.target.value)} value={courseTitle} placeholder='Type here' className='ouline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' required />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <button type='submit' className='bg-black text-white w-max pu-2.5 px-8 rounded my-4 cursor-pointer'
        >ADD</button>
      </form>
    </div>
  )
